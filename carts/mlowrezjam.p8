...
function dynamic:set_vel(grav)  
  ...
  if grav then
    self.vel.y=approach(self.vel.y, self.term_vel, self.grav)
  else
...
function enemy:dead()
  ...
  if  self.t == ceil(self.death_time/2) then
    local midx,midy=(self.hitbox.xl+self.hitbox.xr)/2,(self.hitbox.yb+self.hitbox.yt)/2
    drop_entity(ceil(self.coindrop*level_params[current_level].hm),self.pos.x+midx,self.pos.y+midy,coin)
    if(rnd(1)<self.clifedrop)drop_entity(1,self.pos.x+midx,self.pos.y+midy,life)
  end
...
function enemy:damage(s)
  ...
  if self.health <=0 then shake+=2 self:become("dead") end
end
...
function player:update()
  self.grounded=solid_at(self.pos.x+self.hitbox.xl, self.pos.y+self.hitbox.yt,
...
function player:walking()
  ...
  -- jump
  if btn_press(2) and self.grounded then
...
function player:attacking_charge()
  ...
  if self.vel.x == 0 then
    invoke(function() if scene_player then scene_player.inv=false end end,10)
    self.attk_box.dmg=1
    self:become("walking")
    if self.attk_box~=nil then
      e_remove(self.attk_box)
...
function coin:collide(e)
  if e:is_a("player")then
    if (rp_coin_collected) rp_coin_collected+=1
    self.done=true
  elseif e:is_a("coin")then
...
function chest:open()
  if self.obj then 
    if self.obj:is_a("coin") then
      drop_entity(20*level_params[current_level].hm,self.obj.pos.x,self.obj.pos.y,coin)
    else 
      e_add(self.obj)
    end
  end
...
function dmg_potion:update()
  if(scene_player)self.cost=flr(50 + frac(dmg_mod)*250)
end
...
function dmg_potion:collide(e)
  if btn_press(5) and rp_coin_collected >= self.cost then
    rp_coin_collected-=self.cost
    e.item=self
    self.done=true
  end
end
...
function dmg_potion:render()
  ...
  print(self.cost,self.pos.x+1,self.pos.y-6,9)
end
...
function health_potion:update()
  if(scene_player)self.cost=flr(50 + (scene_player.maxh-4)*50)
end
...
function health_potion:use()
  ...
  if (scene_player and scene_player.maxh<7) scene_player.maxh+=1
  if (scene_player) scene_player.health=scene_player.maxh
end
...
function smoke:init()
  ...
  if not self.r then self.r=rnd(1)+0.5 end
end
...
level=entity:extend({
 ...
 function level:init()
  ...
  local en=ceil(#enemy_pool*lp.pe)
  ...
  for i=0,7 do
    for e in all(r_entities[i]) do
      if debug then
        local ec=c_get_entity(e)
        if ec.b then
          rectfill(ec.b.xl,ec.b.yt,ec.b.xr-1,ec.b.yb-1,8)
        end
      end
...
starting_room_type = 1
number_of_room_types = 2
...
room_layouts_coord = {[r_not_solution] = {x=96, y=0, n=5},
...
level_params = {[1]={sx=3,sy=3,pe=0.3,pl=0.2,hm=1},
                ...
                [7]={sx=6,sy=5,pe=0.6,pl=0.5,hm=1.8},
                [8]={sx=7,sy=5,pe=0.6,pl=0.6,hm=2},
                [9]={sx=8,sy=6,pe=0.7,pl=0.6,hm=2.1},
                [10]={sx=9,sy=7,pe=0.8,pl=0.7,hm=2.2}}
        ...
        for j=0,room_res_x-1 do
            ...
            for my=0,screen_res_y-1 do
                for mx=0,screen_res_x-1 do                    
                    local t = mget(coord.x + mx, coord.y + my + screen_res_y * rnd_layout)
                    mset(j * screen_res_x + mx + cellx, i * screen_res_y + my + celly, t)
                end
            end
        end
    end
...
function draw_ui()
  ...
  sspr(8,0,4,4,49,61)
  print(rp_coin_collected,53,59,7)
end
...
function draw_endgame()
  cls()
  local coins = "coins lost: " .. tostr(rp_coin_collected)
...
function draw_level()
  ...
  camy += (scene_player.pos.y-32-camy)/cam_vel
...
function draw_title()
  ...
  local msg="press x to start"
  print(msg, 32 - (#msg/2)*4,32)
end
...

...

...

...

...
__sfx__

...

...
0111000037074370750000000000000003704437045000000000000000370343703500000000000000037024370250000000000000003701437015000000000000000000000000000000103730c2730c2730c273
__music__
01 28696a44
00 28692a6b
00 2842292b
